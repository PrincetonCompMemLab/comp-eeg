function [featData, labels, winTime, options, featOptions] = ...
    preprocPipeline(sub, experi, dataRoot, varargin)
...
% Run ICA and remove first two components
...
if options.doBase
    if ~exist([saveInterDir saveFname '_Epochs_Base.set'], 'file')
...
% Run ICA to remove blinks
if options.runICA && ~exist([saveInterDir saveFname '_ICA1-2.set'], 'file')
    fprintf('Running ICA\n');
    EEG = pop_runica(EEG, 64, 'icatype', 'sobi');
    EEG = eeg_checkset( EEG );
    fprintf('Removing Blinks\n');
    EEG = pop_subcomp( EEG, [1 2], 0);
    EEG.setname=[EEG.setname '_ICA1-2'];
    saveFname = [saveFname '_ICA1-2'];
    EEG = eeg_checkset( EEG );
    EEG = pop_saveset( EEG, 'filename',[saveFname '.set'],'filepath',saveInterDir);
elseif options.runICA && exist([saveInterDir saveFname '_ICA1-2.set'], 'file')
    saveFname = [saveFname '_ICA1-2'];
    EEG = pop_loadset('filename',[saveFname '.set'],'filepath', saveInterDir);
end
